...
雪崩：大量缓存在同一时间过期，导致直接访问数据库
    随机过期，或者提前预热，构建高可用的缓存集群

击穿：热点key在高并发下瞬时过期，导致请求打到数据库
    加互斥锁，或者设置热点key永不过期
...
缓存空值：缓存中存储空值，当缓存中不存在时，返回空值，而不是报错
布隆过滤器：在缓存和数据库中间设置一个过滤器； 当数据持久化到数据库时，添加key到布隆过滤器
                                            查询时，现在布隆过滤器，判断key是否存在；可能存在就放行，一定不存在就返回空值

分布式锁：只允许一个线程进行数据库的查询，其他线程进行等待重试。获取成功的线程进行缓存的重建；没有进行查询的线程等待后查询缓存

构建高可用的缓存集群：采用哨兵模式或者集群模式，防止单一redis 宕机导致整个缓存服务的崩溃
